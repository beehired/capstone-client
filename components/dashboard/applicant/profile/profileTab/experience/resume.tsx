...
export default function Resume({ id }: { id: string }) {
    ...
    const { data: ResumeData, isLoading: ResumeisLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const { getAllResumyByProfileID } = await GraphQLRequest(GetMyResumeList, {
                profileId: id
            })
    ...
    return (
        <div className={styles.container}>
                ...
                deleteToggle ? <Dialog>
                    <Prompt title='Do you want to delete?' icon={<TbAlertCircleFilled size={23} />}>
                        <div className={PromptStyles.header}>
                            <span>Are you sure you want to delete this resume file? Once deleted, it cannot be recovered, and you will lose all associated information. Please confirm if you wish to proceed with the deletion.
                            </span>
                        </div>
            ...
            {
                isEmpty(ResumeData) ? ResumeisLoading ? <Spinner /> : fileUpload ?
                            ...
                            {({ getRootProps, getInputProps }) => (
                                <section>
                                    <div {...getRootProps()}>
                                        ...
                                        <p className={RegularPoppins.className}>Drag and Drop your resume, or click to select files</p>
                                    </div>
                                </section>
                            )}
                        </Dropzone>
                    ...
                    ResumeData.map(({ title, resumeID, createdAt }: { title: string, resumeID: string, createdAt: any }) => (
                        <div className={styles.file} key={resumeID}>
                            <div className={styles.info}>
                                ...
                                <div className={styles.details}>
                                    ...
                                    <span className={RegularPoppins.className}>Uploaded On: {Formatter(createdAt)}</span>
                                </div>
                            </div>
                ...
                fileUpload ?
                    <div className={styles.formContainer}>
                        <form onSubmit={onHandleSubmit}>
                            <CancelBtn onClose={removedFileUpload} />
