...
import { DeleteProfileTheme, UpdateProfileTheme } from '@/util/Mutation/theme.mutation';
...
export default function ThemeCard({ theme, image, createdAt, themeID }: { themeID: string, theme: string, image: string, createdAt: any }) {
...

            {
                toggleDelete ? <Dialog>
                    <Prompt title='Do you wish to Continue' icon={<TbAlertCircleFilled size={23} />}>
                        <span className={`${PromptStyles.text} ${RegularPoppins.className}`}>Are you sure you want to delete this theme? Once removed, it cannot be recovered, and any related customization will be lost. Please confirm if you wish to proceed with the deletion of this theme.</span>
                ...
                toggleEdit ? <Dialog>
                    <Prompt title='Edit Theme'>
                        <div className={PromptStyles.body}>
                            ...
                            <InputV1 name='theme' errors={errors.theme} placeholder='Theme name' touched={touched.theme} type='text' value={values.theme} onChange={handleChange} />
                        </div>
                        <div className={PromptStyles.footer}>
