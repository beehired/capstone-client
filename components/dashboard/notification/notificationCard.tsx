...
import { ArchiveNotification, UpdateNotification } from '@/util/Mutation/notification.mutation'
...
export default function NotificationCard({ notificationID, title, date, application, status, name }: any) {
    ...
    return (
        <div className={status ? `${styles.container} ${styles.unread}` : `${styles.container}`}>
            {
                toggle ? <Dialog>
                    <div className={styles.dialog}>
                        <div className={styles.dialogContainer}>
                            ...
                            <div className={styles.applicantHeader}>
                                ...
                                {application ? <div className={styles.avatar}>

                                    {application ?
                                        <Image src={isEmpty(application.user?.myProfile?.avatar?.media) ? DefaultImage : application.user?.myProfile.avatar?.media} fill objectFit='cover' objectPosition='center' alt="" /> : null}

                                </div> : <>  <Image src={Logo} alt="" width={150} height={150} objectFit='cover' objectPosition='center' />
                                </>}
                                ...
                                <div className={styles.info}>
                                    {application ? <>
                                        ...
                                        <span>Applicant ID: {application.id} | Application Score: {application.score?.score}%  | <a href={`${application?.resume.resume}`} target="_blank">View my Resume</a></span>
                            ...
                            <div className={styles.applicantBody}>
                                {application ? <>
                                    ...
                                    {!application.jobPost ? <div>
                                        ...
                                        <>
                                            <h2 className={RegularPoppins.className}>{application.jobPost.title}</h2>
                                            <div>
                                    ...
                                    <span>A job post has been deleted due to multiple reports of policy violations. This action is final, and all associated data has been permanently removed. If you believe this was done in error or need further assistance, please contact our support team for more details.</span>}
            ...
            <div onClick={onHandleToggle} className={styles.profile}>
                <div className={styles.info}>
                    <div className={styles.avatar}>
                        ...
                        {application ?
                            <Image src={isEmpty(application.user?.myProfile.avatar?.media) ? DefaultImage : application.user?.myProfile.avatar?.media} fill objectFit='cover' objectPosition='center' alt="" /> :
                            <Image src={Logo} alt="" fill objectFit='cover' objectPosition='center' />
                        }
