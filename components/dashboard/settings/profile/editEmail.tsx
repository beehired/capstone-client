...
export default function EditEmail({ close }: any) {
    ...
    const { handleSubmit, handleChange, errors, values, touched, isSubmitting } = useFormik({
        initialValues: {
            email: "",
            userId: user?.id
        },
        ...
        <div className={styles.container}>
            <Prompt title='Change Email Addess'>
                <div className={PromptStyles.header}>
                    <InputV1 name='email' onChange={handleChange} placeholder='johndoe@example.com' type='text' value={values.email} errors={errors.email} touched={touched.email} />
                </div>
