...
export default function EducationCard({ educationID, degree, endMonth, endYear, school, startMonth, startYear, study }: Education) {
        ...
        <div key={educationID} className={styles.container}>
            {
                editToggle ?
                    <Dialog>
                        <EducationEdit id={educationID} value={editToggle} setValue={setEditToggle} />
                    </Dialog> : null
            }
            {
                deleteToggle ? <Dialog>
                    <Prompt title='Do you want to delete?' icon={<TbAlertCircleFilled size={23} />}>
                        <div className={styles.header}>
                            <span>Are you sure you want to delete this educational background from your profile? Once removed, all related information, such as your degree, will be permanently deleted and cannot be restored. Please confirm if you{"'"}d like to proceed with this action.</span>
                        </div>
            ...
            <div className={styles.header}>
                <h2 className={`${RegularPoppins.className} ${styles.title}`}>{school}</h2>
