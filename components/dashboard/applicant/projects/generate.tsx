...
export default function GenerateReport({ onClose, id }: any) {
    ...
    const mutation = useMutation({
        ...
        mutationFn: async (inputValues: { userId: string, startDate: string, endDate: string }) => {
    ...
    const datas = mutation ? mutation.data?.generateProjectOrganizer.map(({ projectOrganizerID, title, amount, status, startDate, endDate, createdAt, company: { companyName } }: any) => {
        return {
            title,
            amount,
            status,
    ...
    const getTotalAmount = mutation.data?.generateProjectOrganizer.reduce((a: any, b: any) => (a + b.amount), 0)
            ...
            {mutation.data ? <div className={styles.generatedReport}>
                ...
                <div className={styles.generateBody}>
                    <table>
                        ...
                        <tbody>
                            {mutation.data.generateProjectOrganizer.map(({ projectOrganizerID, title, amount, startDate, status, endDate, createdAt, company: { companyName } }: any) => (
                                <tr key={projectOrganizerID}>
                                    ...
                                    <td>
                                        {format(new Date(startDate), "MMMM dd, yyy")}
                                    </td>
                                    <td>
                                        {format(new Date(endDate), "MMMM dd, yyy")}
                                    </td>
                                    <td>
                                        {format(new Date(createdAt), "MMMM dd, yyy")}
                                    </td>
                                    <td>
                                        {companyName}
                                    </td>
                ...
                <div className={styles.generateFooter}>
                    <CSVLink headers={headers} filename={''} style={{ width: "100%", height: "50px", display: "flex", justifyContent: "flex-end" }}
                        data={[...datas,
                        { title: "", amount: "" },
                        {
                            title: "Total Amount", amount: getTotalAmount
                        }]}>
                        <button type="submit">
                            <span>Download</span>
                        </button>
