    ...
    return (
        <div className={styles.container}>
                ...
                editToggle ? <Dialog>
                    <Prompt title='Verify Freelancer Status'>
                        <div className={PromptStyles.header}>
                            <span>Are you sure you want to verify this freelancer? Once verified, this action cannot be undone. Verified freelancers gain increased visibility and credibility on our platform, and the verification status cannot be reverted to unverified. Please confirm that all freelancer details are accurate and up-to-date before proceeding. If you’re ready to make this change, click ‘Confirm.’ Otherwise, click ‘Cancel’ to go back.
                            </span>
                        </div>
            ...
            <div className={styles.header}>
                <div className={styles.info}>
                    <div className={styles.avatar}>
                        <Image src={DefaultImage} alt="" fill objectFit='cover' objectPosition='center' />
                    </div>
                    <h2 className={RegularPoppins.className}>{data?.myProfile.firstname} {data?.myProfile.lastname}</h2>
                </div>
                {data?.verified ? null :
            ...
            <div className={styles.tabView}>
                {isLoading ? <Spinner /> :
                    data?.requirement.map(({ requirementsID, requirement, type, createdAt }: any) => (
                        <FreelancerDocu key={requirementsID} requirement={requirement} type={type} createdAt={createdAt} />
                    ))}
            </div>
        </div>
    )
