...
import { useMutation as TansStackMutation, useQuery } from '@tanstack/react-query';
...
import { AddProfileHeader, DeleteProfileHeader } from '@/util/Mutation/profile.mutation';
...
export default function ProfileHeader({ id }: { id: string }) {
    ...
    const { data: ProfileHeaderData, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const { getProfileByUser } = await GraphQLRequest(GetProfileHeaderByUser, {
                userId: user?.id
            })
    ...
    return (
        <div className={styles.container}>
                ...
                deleteToggle ? <Dialog>
                    <Prompt title='Do you want to delete?' icon={<TbAlertCircleFilled size={23} />}>
                        <div className={styles.header}>
                            <span>Are you sure you want to delete this image? Once you confirm, the image will be permanently removed and cannot be recovered. Please confirm if you wish to proceed with the deletion.</span>
                        </div>
            ...
            {isLoading ? <Spinner /> : isEmpty(ProfileHeaderData?.header?.media) ?
                <div className={styles.fileUpload}>
                                ...
                                {({ getRootProps, getInputProps }) => (
                                    <section>
                                        <div {...getRootProps()}>
                                            ...
                                            <p className={RegularPoppins.className}>Drag and Drop your image, or click to select image</p>
                                        </div>
                                    </section>
                                )}
                            </Dropzone>
                    ...
                    <div>
                        <Image src={ProfileHeaderData?.header?.media} alt="" fill objectFit="contain" objectPosition='center' priority blurDataURL={ProfileHeaderData?.header?.media} />
                    </div>
                ...
                fileUpload ?
                    <div className={styles.formContainer}>
                        <form onSubmit={onHandleSubmit}>
                            ...
                            <PrimaryButton type='submit' loading={loading ? true : false} name='Submit' />
                        </form></div> : null
            }
            <ToastNotification />
        </div >
