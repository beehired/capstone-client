...
import { PaypalCancelSubscription, UpgradePaypalSubscription } from '@/util/Mutation/paypal.mutation';
...
export default function PlansAndBillings() {
    ...
    const UpgradeSMutation = useMutation({
        ...
        mutationFn: async (inputValues: { subscriptionId: string, userId: string }) => {
            ...
            {toggle ? <Dialog>
                <Prompt title='Cancel Subscriptions'>
                    <div className={PrompStyles.header}>
                        <span>Canceling your subscription will revert your account to the Basic plan, limiting access to premium features. Are you sure you want to proceed?</span>
                    </div>
                    <div className={PrompStyles.footer}>
                        <CancelBtn onClose={onHandleToggle} />
                        <form onSubmit={CancelFormik.handleSubmit}>
                            <PrimaryButton loading={CancelFormik.isSubmitting ? true : false} name='Confirm' type='submit' />
                        </form>
                    </div>
                </Prompt>
            </Dialog> : null}
                ...
                upgrade ?
                    <Dialog>
                        <Prompt title='Upgrade Plan'>
                                ...
                                <PayPalScriptProvider options={{
                                    ...
                                    <PayPalButtons
                                        ...
                                        onApprove={(data): any => {
                                            ...
                                            UpgradeFormik.setFieldValue("subscriptionId", data.subscriptionID)
                            ...
                            <div className={PrompStyles.footer}>
                                <CancelBtn onClose={onHandleUpgrade} />
                            </div>
                        </Prompt>
                    </Dialog> : null
            }
                        ...
                        data?.user?.plan === "PRO" ? <>
                            ...
                            <div className={styles.footer}>
                                <button onClick={onHandleToggle}>
                                    <span className={RegularPoppins.className}>Cancel Subscription</span>
                                </button>
                            </div>
                        </> :
                            <>
