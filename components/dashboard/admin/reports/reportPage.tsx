        ...
        <div className={styles.container}>
            {
                toggle ? <Dialog>
                    ...
                    <Prompt title='Do you want to delete this Job Post?' icon={<TbAlertCircleFilled size={23} />}>
                        <div className={PromptStyles.header}>
                            <span className={RegularPoppins.className}>Are you sure you want to delete this job post? This action is permanent and cannot be undone. All associated data, including applications, will be permanently removed. Please confirm to proceed or cancel to keep the job post active.</span>
                        </div>
            ...
            {isLoading ? <Spinner /> : data?.jobPost.map(({ reportID, title, description, createdAt, duration, experience, location, endDate, status }: any) => (
                <div key={reportID}>
                    <div className={styles.head}>
                        <h2>{title}</h2>
                        <button onClick={onHandleToggle}>
                            <span className={RegularPoppins.className}>Delete Job Post</span>
                        </button>
                    </div>
