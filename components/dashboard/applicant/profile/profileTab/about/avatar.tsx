                ...
                deleteToggle ? <Dialog>
                    <Prompt title='Do you want to delete?' icon={<TbAlertCircleFilled size={23} />}>
                        <div className={styles.header}>
                            <span>Are you sure you want to delete this avatar? This action cannot be undone. Please confirm if you would like to proceed</span>
                        </div>
            ...
            <div className={styles.info}>
                <div className={styles.avatar}>
                    {isLoading ? <Spinner /> :
                        ...
                        <Image src={isEmpty(data?.avatar?.media) ? DefaultImage : data?.avatar?.media} alt="" priority blurDataURL={data?.avatar?.media} fill objectFit='cover' objectPosition='center' />
                    }
                </div>
                {/* <h2 className={RegularPoppins.className}>{isLoading ? <Spinner /> : `${data?.firstname} ${data?.lastname}`}</h2> */}
            </div>
            <div className={styles.btnGrp}>
                {isEmpty(data?.avatar) ? <button onClick={onHandleAddToggle} className={styles.change}>
                    <span className={RegularPoppins.className}>{data?.avatar ? `Change Profile` : `Upload your image`}</span>
                </button> : null}
                {isEmpty(data?.avatar) ?
                    null : <button onClick={onHandleDeleteToggle} className={styles.removed}>
                        <TbTrash size={22} />
                    </button>}
            </div>
        </div>
