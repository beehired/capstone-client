...
import { TbAlertCircle, TbAlertCircleFilled, TbEdit, TbTrash } from 'react-icons/tb';
...
export default function WorkCard({ title, description, companyName, location, startMonth, createdAt, endMonth, endYear, employmentType, locationType, startYear, portfolioID, skills
}: Props) {
        ...
        <div className={styles.container}>
                    ...
                    <Dialog>
                        <Prompt title='Do you want to delete?' icon={<TbAlertCircleFilled size={23} />}>
                            <div className={PromptStyles.header}>
                                <span>Are you sure you want to delete this work experience from your profile? Once removed, all associated details will be permanently deleted and cannot be recovered. Please confirm if you{"'"}d like to proceed with this action.
                                </span>
                            </div>
                            <div className={PromptStyles.footer}>
                                {isSubmitting ? null : <CancelBtn onClose={onhHandleDeleteToggle} />}
            ...
            <div>
                <div className={styles.header}>
                    <h2 className={`${RegularPoppins.className} ${styles.title}`}>{title}</h2>
                ...
                <div className={styles.skills}>
                    {skills.slice(0, 5).map(({ skills }: { skills: any }) => (
                        <div key={skills}>
                            <span className={RegularPoppins.className} key={skills}>{skills}</span>
                        </div>
                    ))} {skills.length > 5 ? `${skills.slice(5, skills.length).length}+ more` : null}
                </div>
            </div>
        </div>
    )
