...
export default function FreelancerAccountSecurity() {
    ...
    const { values, errors, touched, handleSubmit: ChangePass, handleChange, isSubmitting: isPassword } = useFormik({
        initialValues: {
            newpass: "",
            retypepass: ""
        },
                        ...
                        <Prompt title='Change Password'>
                            <div className={PromptStyles.inputHeader}>
                                <InputV2
                                    ...
                                    type={onToggle ? "text" : "password"} name='newpass' placeholder='New Password' value={values.newpass} onChange={handleChange}
                                    errors={errors.newpass} touched={touched.newpass}
                                />
                                <InputV2
                                    toggle={onToggle}
                                    onToggle={setToggle}
                                    icon={<TbLock size={23} />}
                                    icon2={onToggle ? <TbEyeOff size={23} /> : <TbEye size={23} />}
                                    type={onToggle ? "text" : "password"} name='retypepass' placeholder='Re-Type Password' value={values.retypepass} onChange={handleChange}
                                    errors={errors.retypepass} touched={touched.retypepass}
                                />
                            </div>
                            <div className={PromptStyles.footer}>
