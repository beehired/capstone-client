            ...
            <div className={styles.applicationContainer}>
                ...
                {isLoading ? (
                    ...
                    <>
                        {isEmpty(data) ? (
                            ...
                            data.map(({ resumeID, title }: { resumeID: string; title: string }) => (
                                <div className={styles.resumeCard} key={resumeID}>
                                    <div>
                                        <h3 className={MediumPoppins.className}>{title}</h3>
                                    </div>

                                    <InputRadio name="resumeId" onChange={handleChange} value={resumeID} />
                                </div>
                            ))
                        )}
                    </>
                ...
                <div className={styles.formContainer}>
                    <form onSubmit={handleSubmit}>
                        {isSubmitting ? null : <CancelBtn onClose={close} />}
